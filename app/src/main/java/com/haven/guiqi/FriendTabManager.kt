...
class FriendTabManager(
    ...
    fun showAddFriendDialog() {
        ...
        AlertDialog.Builder(activity)
            ...
            .setPositiveButton("添加") { _, _ ->
                val name = input.text.toString().trim().ifEmpty { "新好友" }
    ...
    /** 通过编码找回已删除的好友（重新创建一个使用该编码的好友） */
    private fun showAddByCodeDialog() {
        val input = EditText(activity).apply {
            hint = "输入好友编码（如 HV-A3F8）"
            setPadding(dp(16), dp(12), dp(16), dp(12))
            textSize = 15f
        }
        AlertDialog.Builder(activity)
            .setTitle("通过编码找回")
            .setMessage("输入之前好友的编码，可以找回关联的记忆、日记、梦境等数据。")
            .setView(input)
            .setPositiveButton("找回") { _, _ ->
                val code = input.text.toString().trim()
                if (code.isEmpty()) {
                    Toast.makeText(activity, "编码不能为空", Toast.LENGTH_SHORT).show()
                    return@setPositiveButton
                }
                // 检查是否已有这个编码的好友
                val existing = friendStorage.getFriend(code)
                if (existing != null) {
                    Toast.makeText(activity, "「${existing.name}」已在好友列表中", Toast.LENGTH_SHORT).show()
                    return@setPositiveButton
                }
                // 用这个编码创建好友
                val friend = friendStorage.addFriendWithId(code, "找回的好友")
                Toast.makeText(activity, "已找回好友\n编码: ${friend.visibleCode}", Toast.LENGTH_SHORT).show()
                refresh()
                onRefresh()
            }
            .setNegativeButton("取消", null)
